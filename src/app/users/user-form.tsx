  ...
  const onChangeFieldStatus =
    (status: string) => (event: React.ChangeEvent<HTMLInputElement>) => {
      if (event.target.checked) {
        setForm((prev) => ({
          ...prev,
          status: status as UserForm["status"],
        }));
      }
    };
  ...
  return (
    <form onSubmit={(event) => onSubmit(event, form)}>
      <div className="space-y-4">
        ...
        <label className="block">
          <span className="block text-left text-sm font-medium mb-1">
            Email
          </span>
              ...
              return (
                ...
                >
                  <input
                    ...
                    defaultChecked={form.status === status}
                    onChange={onChangeFieldStatus(status)}
                  />
